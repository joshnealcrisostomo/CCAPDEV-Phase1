document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!postId || !posts[postId]) {
        document.getElementById("visitor-post-container").innerHTML = "<p>Post not found.</p>";
    ...
    const templateSource = document.getElementById("visitor-post-template").innerHTML.trim();
    const template = Handlebars.compile(templateSource);
    const compiledHtml = template(postData);

    // Insert compiled HTML into page
    document.getElementById("visitor-post-container").innerHTML = compiledHtml;
